...
For example artifacts used to build 1.8.0 RC1 should be tagged `v1.8.0-1`.
Releases are **not** created in GitHub for RCs.

Example for creating an annotated tag on the checked out commit for RC1 for `v1.8.0`:
```shell script
git tag -a v1.8.0-1 -m "RC1 for v1.8.0"
...
- Click on the version that is about to be released, i.e. `1.8.0`
...
So `0.12.2` or `1.8.0` **without** any additional references like a `RC1` or something similar.
...
As an example when releasing the first release candidate for `1.8.0` the json file should contain version and tag information like below:
```json
{
  "release": {
    "version": "1.8.0"
...
```shell script
gpg --local-user <email-address> --armor --output apache-yunikorn-1.8.0-src.tar.gz.asc --detach-sig apache-yunikorn-1.8.0-src.tar.gz
```
This will create the signature in the file: `apache-yunikorn-1.8.0-src.tar.gz.asc`
Verify that the signature is correct using:
```shell script
gpg --verify apache-yunikorn-1.8.0-src.tar.gz.asc apache-yunikorn-1.8.0-src.tar.gz
```
...
```shell script
shasum -a 512 apache-yunikorn-1.8.0-src.tar.gz > apache-yunikorn-1.8.0-src.tar.gz.sha512
```
This will create the checksum in the file: `apache-yunikorn-1.8.0-src.tar.gz.sha512`
Verify that the checksum is correct using:
```shell script
shasum -a 512 -c apache-yunikorn-1.8.0-src.tar.gz.sha512
```
...
Create a release directory based on the version, i.e. `1.8.0`, add the three files to directory.
Commit the changes.
...
### Tag for release
Once the release has passed voting, git tags and GitHub releases should be created for final release tag, example `v1.8.0`
The final tag must point to the same commit as the approved release candidate tag, example `v1.8.0-1`
...
```shell script
git tag -a v1.8.0 -m "Apache YuniKorn v1.8.0"
...
- helm package: example `yunikorn-1.8.0.tgz`
- provenance or signature file: example `yunikorn-1.8.0.tgz.prov`
...
```shell script
shasum -a 256 yunikorn-1.8.0.tgz
```
...
- Create the release announcement to be referenced from download page on the website.
  The release announcement is a markdown file based on the version: `1.8.0.md`.
...
under `versioned_docs`, e.g `versioned_docs/version-1.8.0`.
...
```
src/pages/release-announce/1.8.0.md
```
...
* 1 release announcement using a link in the form of `/release-announce/1.8.0`
